...
| `browser.launchOptions` | `Record<string, unknown>` | `{}` | 透传引擎启动参数 |
| `browser.contextOptions` | `Record<string, unknown>` | `{}` | 透传上下文参数 |
...
| `diff.ignoreRegions[]` | `Array` | — | 对比时忽略的区域 |
...
|------|------|------|
| `name` | `string` | 插件名称 |
| `options` | `Record<string, unknown>` | 插件自定义选项 |
